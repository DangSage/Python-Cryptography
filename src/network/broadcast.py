...
def broadcast_listener(socket, shutdown_event):
    print("Listening for broadcast from socket ", socket.getsockname())
    socket.settimeout(3)  # Set a timeout
    try:
        while not shutdown_event.is_set():
            ...
            if email_exists and ng.potential_contact[3] not in ng.ignore_bcast_port:
                list = (
                    get_user_name_from_list(),
                    gl.USER_EMAIL,
                    ng.tcp_listen,
...
def broadcast_sender(port, shutdown_event):
